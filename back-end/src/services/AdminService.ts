...
class AdminService {
  ...
  async login(username: string, passwordHash: string, adminIp: string): Promise<{ data?: Admin; errors: string[] }> {
    try {
      const admin = await this.repository.findAdminByUsername(username, adminIp);
